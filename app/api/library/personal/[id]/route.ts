...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await supabaseAdmin.storage.from("personal-photos").remove([row.storage_path]);

  const { error } = await supabaseAdmin.from("user_personal_photos").delete().eq("id", params.id);
